...
class Board(QtWidgets.QFrame):
    ...
    def __init__(self, parent):
        ...
        self.wave_delay = 20  # count of iteration between waves
    ...
    def timerEvent(self, a0: QtCore.QTimerEvent) -> None:
        if a0.timerId() == self.timer.timerId():
            for i in range(len(self.enemies)):
                for j in range(self.enemies[i].velocity):
    ...
    def paintEvent(self, a0: QtGui.QPaintEvent) -> None:
        ...
        if self.fire_counter % self.fire_delay == 1:  # if it's fire iteration
            for enemy, archer in self.archersAttacks:
                ...
                alpha = math.atan2(abs(y2 - y1), abs(x2 - x1))  # angle of arrow rotation
                print(alpha, int(x1), int(y1), int(x2), int(y2))
                # t = QTransform().rotate(alpha)
                # arrow = archer.arrow.transformed(t)
                # self.draw_rect(painter, rect.left() + int(x1), board_top + int(y1), arrow)
        ...
        for enemy in self.enemies:
            ...
            if coord[0] >= self.HEIGHTINBLOCKS * 3 + 3 or coord[1] >= self.WIDTHINBLOCKS:
                self.enemies.remove(enemy)
            else:
                i = coord[0]
                j = coord[1]
