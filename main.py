...
class Board(QtWidgets.QFrame):
    ...
    def paintEvent(self, a0: QtGui.QPaintEvent) -> None:
        ...
        # self.HEIGHTINBLOCKS = int(rect.height() / self.tile_width())
        # print(self.HEIGHTINBLOCKS)
        ...
        if self.fire_delay % 6 == 0:  # if it's fire iteration
            for archer, enemy in self.archersAttacks:
                ...
                x1 = self.x_coord(archer_coord[0], archer_coord[1])
                y1 = self.y_coord(archer_coord[1])
                x2 = self.x_coord(enemy_coord[0], enemy_coord[1])
                y2 = self.y_coord(enemy_coord[1])
                painter.drawLine(int(x1), int(y1), int(x2), int(y2))
                self.archersAttacks.pop(0)
            for wizard, enemy in self.wizardsAttacks:
                wizard_coord = self.wizards[wizard].position
                enemy_coord = self.enemies[enemy].position
                x1 = self.x_coord(wizard_coord[0], wizard_coord[1])
                y1 = self.y_coord(wizard_coord[1])
                x2 = self.x_coord(enemy_coord[0], enemy_coord[1])
                y2 = self.y_coord(enemy_coord[1])
                painter.drawLine(int(x1), int(y1), int(x2), int(y2))
                self.wizardsAttacks.pop(0)
